...
function renderPages() {
  ...
  pages.forEach((page, index) => {
    ...
    thumb.className = "page-thumb" + (index === currentPage ? " active" : "");
    thumb.textContent = index + 1;

    thumb.onclick = () => { currentPage = index; currentImage = null; redraw(); };
...
window.onmouseup = () => { if(drawing || dragging || resizing || rotating){ saveHistory(); saveToFirebase(); } drawing=false; dragging=false; resizing=false; rotating=false; };
...
addPage.onclick = () => { pages.push({strokes:[], images:[]}); currentPage = pages.length-1; redraw(); saveToFirebase(); };
...
pagesRef.on("value", snap => {
  ...
  } else { pages=[{strokes:[],images:[]}]; currentPage=0; saveToFirebase(); }
  redraw();
});
